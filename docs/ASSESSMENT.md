...
**已完成：WMI 訂閱持久化（OBJECTS.DATA）**
- 完整 CIM repository 解析（INDEX.BTR/MAPPING + class/instance）龐大且版本敏感、做錯會捏造 findings。故 `WmiPersistenceParser.cs` 比照 PyWMIPersistenceFinder 採**triage 級字串萃取**：抓可讀 ASCII run，比對**實例**樣式（`<Class>EventConsumer.Name="..."`、`__EventFilter` + WQL 查詢、binding 參照），**刻意略過** class 定義／provider 註冊（只有屬性宣告、無實例值）→ 報訂閱全貌而非 schema 雜訊。`WmiProvider.cs`（opt-in）每筆一個 `Persistence` 事件（filter/consumer/binding），OBJECTS.DATA 匯出進 `raw/wmi/`。
- **驗證（真實 42 MB OBJECTS.DATA）**：正確還原預設 `SCM Event Log` 訂閱（binding：consumer `SCM Event Log Consumer` ← filter `SCM Event Log Filter`；filter WQL `select * from MSFT_SCMEventLogEvent`），且**未**把 12 個 class 定義誤報為 findings；end-to-end agent 3 筆、OBJECTS.DATA 打包 `copied=1/skipped=0`。
- 接線：UI `File ▸ Load WMI...`、Agent `--wmi=`。測試 `WmiPersistenceParserTests`（4：合成惡意 binding、schema 去噪、ASCII run、真實 repository）。
- **界線**：不解 consumer 的 command-line/script **payload**（需真 CIM 解析如 python-cim）—— 還原的 consumer 名稱/binding 是 pivot 線索，非完整動作。詳見 `LIMITATIONS §25`。

**P2 全數完成**（SRUM / BITS / WMI 皆已落地並以真實樣本驗證）。剩餘僅 P5 Remote Agent 產品化（大型工程）與 P6 根本盲點（live tool 宿命）。

### P3 — JumpList DestList parser ✅ 已修復（含一個連帶 bug）
原本 StreamName／offset 與真實 DestList 佈局不符、查找永遠 miss → 回退 LNK 時間戳；現已**用真實 v6 樣本 + JLECmd ground truth 逐筆驗證**重寫。
- **正確佈局**（`JumpListDestListParser.cs`）：header 0x20；每筆 entry 固定段（v≥3=0x80、v1=0x70）含 droids、Hostname(0x48)、**EntryNumber(0x58→stream 名，小寫 hex 無補零)**、**LastModified FILETIME(0x64)**、PinStatus(0x70)，其後 2-byte 路徑字元數 + UTF-16 路徑，v≥3 再 4 bytes trailer。逐筆變長前進。
- **連帶修復**：`JumpListProvider` 用了無效的 `RootStorage.OpenRead(path, LeaveOpen)`（OpenMcdf 3.x 只允許注入串流用 LeaveOpen）→ 結構化解析其實一直在丟例外、退回容器事件。改為 `OpenRead(path)`，結構化 JumpList 項目（含 DestList enrich）現在才真正生效。
- **驗證**：`5f7b...`（entry 1 → `C:\Program Files\cursor\Cursor.exe`、host zen9ya0）、`f01b...`（entry d3=211 → `D:\cursor\KDFIR\KAPE`、LastModified 2026-03-12 02:41:14）皆與 JLECmd 吻合。測試 `JumpListDestListTests`（5：合成佈局、變長前進、零時戳、兩個真實樣本）。
- **界線**：DestList 只存 LastModified（存取）時戳；JLECmd 顯示的 CreationTime 來自內嵌 LNK（非 DestList）。v1 佈局以較小固定段 best-effort 支援（樣本為 v6）。
...
**已完成（P6 方向第二步）：live vs offline 交叉比對（把盲點變 tripwire）**
- 根本限制（ring -1/0 說謊、觀察者效應）無法用 user-mode 程式碼消除；但「**比對 live API 說的 vs raw/offline 的真相、差異即竄改/隱藏指標**」可以。新增純函式 `WinDFIR.Core/Analysis/CrossSourceAnomalyDetector.cs`（可單元測試）+ `CrossSourceServiceAnalyzer.cs`。
- **首個應用：服務**。新增 `LiveServiceProvider`（WMI `Win32_Service`，過去完全沒有 live 服務來源）作為 live 半邊；offline 半邊是既有的 raw SYSTEM hive Services。比對後：**hive 有、live API 沒有 = 可能被 hook 隱藏**（`MissingFromLive`）；live 有、hive 沒有 = 可能 memory-only/injected。一律 **Amber「待查」**（非紅「確認 rootkit」），因有良性原因（停用/剛移除）。
- **降噪**：offline 端只比對 user-mode（Win32）服務（WMI 不列舉 kernel driver），用 Type DWORD bits 判定；並**解碼 offline provider 以 hex 呈現的值**（這在實作時抓到一個真 bug：analyzer 原本吃不到 hex 化的 ServiceImagePath/Type，會導致比對永遠空集）。
- **接線**：Agent 在採集後、匯出前自動跑（雙來源齊備才動，否則 no-op 不亂猜）；UI `Advanced ▸ Cross-Source Anomalies...`。匿名事件 `Category=Anomaly`、`Confidence=Low`（明示為 tripwire）。
- **驗證**：對真實 SYSTEM hive（KAPE）萃取出 user-mode 服務並餵 detector，植入「被隱藏的服務」確實命中 `MissingFromLive`；本機 live 端 WMI 解出 319 服務。測試 `CrossSourceAnomalyTests`（7）+ `CrossSourceRealHiveTests`（1）。詳見 `LIMITATIONS §26`。
- **誠實界線**：WMI 本身仍是 user-mode（夠強的 ring-0 可一起騙 WMI 與你）；這抬高對手成本、偵測常見隱藏，但非萬靈丹 —— 目標是「證據源之一 + 竄改 tripwire」。

**已完成（P6 第二個交叉比對）：scheduled task — on-disk XML vs TaskCache registry**
- 常見隱形排程技巧：刪掉 `\Tasks\<name>` XML 躲過 Task Scheduler UI / `schtasks`，但 registry `TaskCache` 仍在（或直接寫 registry）。`CrossSourceTaskAnalyzer` 比對兩邊：TaskCache 有、on-disk XML 沒有 = 可能隱藏。
- 偵測器泛化（加 source label 參數）以複用；新增共用 `OfflineValueDecoder`（service/task 共用 hex 解碼）。
- **過程抓到兩個真 bug**：(1) `TaskCache_Path` 取自被**截斷到 200 字元**的 hex 顯示值 → 長的 `\Microsoft\Windows\...` 路徑被切斷無法對應；改為從**完整 raw bytes** 解碼（順帶讓 TaskCache_Path 變人類可讀）。(2) analyzer 原本用 `Action=="ScheduledTask"` 過濾，但 index 會正規化 Action → 改用 `TaskName` 欄位存在判定。
- **驗證（真實 KAPE Tasks 目錄 + SOFTWARE hive）**：on-disk 232 / TaskCache 263 任務，正規化後**多數對得上**（overlap 測試通過）；植入「只在 TaskCache、無 XML」確實命中 `MissingFromLive`。測試 `CrossSourceTaskTests`（4）。
- 接線：Agent 採集後自動跑 services + tasks 兩個比對；UI 同一個 `Cross-Source Anomalies...` 一次跑兩者。

**已完成（P6 第三、四個交叉比對）：run-key 與 process**
- **Run-key**（`CrossSourceRunKeyAnalyzer`）：live registry API 的 Run 鍵 vs raw hive 的 Run 鍵（HKLM/HKCU），key = hive|valueName。hive 有、live 沒有 = 可能隱藏。**界線**：live registry 預設關閉（forensic policy）→ 預設情況此比對 no-op，需啟用 live registry 才生效；故僅比對 Run scope（live 端不含 RunOnce/Winlogon）。
- **Process**（`ProcessApiCrossCheckProvider`，採集時 provider 而非 index analyzer）：兩個獨立的 live 進程列舉 API —— 原生 `Process.GetProcesses()`（NtQuerySystemInformation 路徑）vs WMI `Win32_Process`。某 PID 只被其中一個 API 看到 = 可能進程隱藏（經典 user-mode 隱藏指標）。**降 FP**：兩個都是 point-in-time 快照，故每個差異都**直接重查確認**（過濾在兩次列舉間啟動/結束的瞬時進程）。
- **驗證（本機 live）**：乾淨主機跑出 **0 個確認異常**（兩 API 一致，無誤報）—— 確認 re-check 有效。測試 `CrossSourceRunKeyTests`（9）。
- **誠實界線**：process 兩個 API 都是 user-mode（ring-0 可一起騙）；要真正 rootkit-resistant 需 raw 記憶體/handle table，目前沒有。這是抬高成本的 tripwire。
- 接線：Agent 採集後自動跑 service/task/run-key 三個 index 比對 + process provider（採集時）；UI menu 一次跑 service/task/run-key（process 屬採集時，無法從載入的快照重跑）。
