...
**P2 剩餘（皆為高風險 ESE/二進位 repository，刻意延後）**：WMI 訂閱 `OBJECTS.DATA`、BITS job DB、SRUDB.dat。需真實樣本驗證，做錯會注入錯誤證據，待明確需求再開。
...
- **剩餘**：跨 volume 合併 MFT 檢視仍未做（跨碟比對需手動切 tab）——屬功能擴充，非截斷可視化。
