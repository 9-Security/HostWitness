...
A live tool cannot guarantee it sees an object a kernel/firmware implant hides, but a discrepancy between what a **live API** reports and what the **raw/offline** source shows is a high-value tampering/hiding indicator. `CrossSourceAnomalyDetector` compares the two views and emits `Category="Anomaly"` events. Four applications ship:
- **Services** — live `LiveServiceProvider` (WMI `Win32_Service`) vs the raw SYSTEM-hive Services.
- **Scheduled tasks** — on-disk Task XML (`%WinDir%\System32\Tasks`) vs the registry `TaskCache`.
- **Run keys** — the live registry API vs the raw hive (HKLM/HKCU `Run`). **No-op unless live registry collection is enabled** (off by default under the forensic policy).
- **Processes** — the native process list (`Process.GetProcesses`, an NtQuerySystemInformation path) vs WMI `Win32_Process`, captured live at collection time. Every discrepancy is **re-confirmed** by directly re-querying the API it appeared to be missing from, to filter out processes that start/exit between the two point-in-time snapshots.

An item in the raw/second source but missing from the live/first view (`MissingFromLive`) is a possible hiding indicator (e.g. a task whose XML was deleted while TaskCache still runs it, or a process hidden from one API); the reverse (`MissingFromOffline`) is a possible memory-only/injected or just-created item. The services/tasks/run-key checks run automatically in the agent (after collection, before export) and on demand in the UI (`Advanced ▸ Cross-Source Anomalies...`); the process check runs as a collection-time provider (it needs the two live APIs, so it cannot be re-run from a loaded snapshot). **Bounds:** findings are advisory **Amber/Low confidence** ("investigate"), never "confirmed rootkit" — benign causes exist (disabled or recently-removed services, collection timing). The offline set is filtered to user-mode (Win32) services because WMI does not enumerate kernel drivers (comparing them would be coverage noise). It is a no-op unless BOTH sources are present (the offline SYSTEM hive needs Administrator/VSS on a live host). Crucially, **WMI is itself a user-mode API**: a sufficiently privileged ring-0/firmware implant can lie to both WMI and the raw read, so this raises the adversary's cost and catches common hiding, but is a tripwire and corroboration aid — not a guarantee. (Presence is compared; image-path value comparison is deferred pending path normalization.)
